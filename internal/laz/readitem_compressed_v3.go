...
// ---------------------------------------------------------------------------
// POINT14 v3 — layered decompression for LAS 1.4 point types 6-10
//
...
type LASreadItemCompressedPoint14v3 struct {
...
func NewLASreadItemCompressedPoint14v3(dec *ArithmeticDecoder, decompressSelective uint32) *LASreadItemCompressedPoint14v3 {
	r := &LASreadItemCompressedPoint14v3{dec: dec}
...
func (r *LASreadItemCompressedPoint14v3) ChunkSizes() error {
...
func (r *LASreadItemCompressedPoint14v3) Init(item []byte, ctx *uint32) error {
	instream := r.dec.GetByteStreamIn()
...
func (r *LASreadItemCompressedPoint14v3) createAndInitModelsDecompressors(ctx uint32, item []byte) {
	c := &r.contexts[ctx]
	if !c.Unused {
		return
	}
	...
	if c.MChangedValues[0] == nil {
		...
		// GPS time
		c.MGPSTimeMulti = r.decGPSTime.CreateSymbolModel(gpstimeMultiTotalV3) // v3: 515 symbols (not 516 as in v2)
...
// C++ original: LASreadItemCompressed_POINT14_v3::read(U8* item, U32& context)
func (r *LASreadItemCompressedPoint14v3) Read(item []byte, context *uint32) error {
...
func (r *LASreadItemCompressedPoint14v3) readGPSTime(c *LAScontextPOINT14) error {
	last := c.Last
...
// ---------------------------------------------------------------------------
// RGB14 v3 — layered decompression for LAS 1.4 RGB (3×uint16)
//
...
type LASreadItemCompressedRGB14v3 struct {
...
func NewLASreadItemCompressedRGB14v3(dec *ArithmeticDecoder, decompressSelective uint32) *LASreadItemCompressedRGB14v3 {
	r := &LASreadItemCompressedRGB14v3{dec: dec}
...
func (r *LASreadItemCompressedRGB14v3) ChunkSizes() error {
	instream := r.dec.GetByteStreamIn()
	buf := make([]byte, 4)
	if err := instream.Get32bitsLE(buf); err != nil {
		return err
...
func (r *LASreadItemCompressedRGB14v3) Init(item []byte, ctx *uint32) error {
	instream := r.dec.GetByteStreamIn()
...
func (r *LASreadItemCompressedRGB14v3) createAndInitModelsDecompressors(ctx uint32, item []byte) {
	c := &r.contexts[ctx]
	if !c.Unused {
		return
	}
...
// C++ original: LASreadItemCompressed_RGB14_v3::read(U8* item, U32& context)
//
// v3 context-switch rule: lastItem only updated to new context when unused.
func (r *LASreadItemCompressedRGB14v3) Read(item []byte, context *uint32) error {
	c := &r.contexts[r.currentContext]
	lastItem := c.LastItem[:]
	...
	if r.currentContext != *context {
		...
		if r.contexts[r.currentContext].Unused {
			...
			lastItem = r.contexts[r.currentContext].LastItem[:] // only when newly initialized
		}
		c = &r.contexts[r.currentContext] // always use new context's models
	}
...
func NewLASreadItemCompressedRGBNIR14v3(dec *ArithmeticDecoder, decompressSelective uint32) *LASreadItemCompressedRGBNIR14v3 {
	r := &LASreadItemCompressedRGBNIR14v3{dec: dec}
	r.requestedRGB = (decompressSelective & LASZIP_DECOMPRESS_SELECTIVE_RGB) != 0
...
func (r *LASreadItemCompressedRGBNIR14v3) ChunkSizes() error {
	instream := r.dec.GetByteStreamIn()
	buf := make([]byte, 4)
	if err := instream.Get32bitsLE(buf); err != nil {
		return err
...
func (r *LASreadItemCompressedRGBNIR14v3) Init(item []byte, ctx *uint32) error {
	instream := r.dec.GetByteStreamIn()
...
func (r *LASreadItemCompressedRGBNIR14v3) createAndInitModelsDecompressors(ctx uint32, item []byte) {
	c := &r.contexts[ctx]
	if !c.Unused {
		return
	}
...
// C++ original: LASreadItemCompressed_RGBNIR14_v3::read(U8* item, U32& context)
//
// v3 context-switch rule: lastItem only updated to new context when unused.
func (r *LASreadItemCompressedRGBNIR14v3) Read(item []byte, context *uint32) error {
	c := &r.contexts[r.currentContext]
	lastItem := c.LastItem[:]
...
// ---------------------------------------------------------------------------
// WAVEPACKET14 v3 — layered decompression for LAS 1.4 wave packet (29 bytes)
//
// Single layer with 4 symbol models + 4 integer compressors.
// Context: LAScontextWAVEPACKET14
// ---------------------------------------------------------------------------

type LASreadItemCompressedWavepacket14v3 struct {
	dec *ArithmeticDecoder
...
func NewLASreadItemCompressedWavepacket14v3(dec *ArithmeticDecoder, decompressSelective uint32) *LASreadItemCompressedWavepacket14v3 {
	r := &LASreadItemCompressedWavepacket14v3{dec: dec}
...
func (r *LASreadItemCompressedWavepacket14v3) ChunkSizes() error {
	instream := r.dec.GetByteStreamIn()
	buf := make([]byte, 4)
	if err := instream.Get32bitsLE(buf); err != nil {
		return err
...
func (r *LASreadItemCompressedWavepacket14v3) Init(item []byte, ctx *uint32) error {
	instream := r.dec.GetByteStreamIn()
...
func (r *LASreadItemCompressedWavepacket14v3) createAndInitModelsDecompressors(ctx uint32, item []byte) {
	c := &r.contexts[ctx]
	if !c.Unused {
		return
	}
...
// C++ original: LASreadItemCompressed_WAVEPACKET14_v3::read(U8* item, U32& context)
//
// v3 context-switch rule: lastItem only updated to new context when unused.
func (r *LASreadItemCompressedWavepacket14v3) Read(item []byte, context *uint32) error {
	c := &r.contexts[r.currentContext]
	lastItem := c.LastItem[:]
...
// ---------------------------------------------------------------------------
// BYTE14 v3 — layered decompression for LAS 1.4 extra bytes (variable count)
//
// One instream/decoder per byte. Each byte gets its own byte-level model.
// Context: LAScontextBYTE14
// ---------------------------------------------------------------------------

type LASreadItemCompressedByte14v3 struct {
	dec *ArithmeticDecoder
...
func NewLASreadItemCompressedByte14v3(dec *ArithmeticDecoder, number uint32, decompressSelective uint32) *LASreadItemCompressedByte14v3 {
	r := &LASreadItemCompressedByte14v3{dec: dec, number: number}
...
func (r *LASreadItemCompressedByte14v3) ChunkSizes() error {
	instream := r.dec.GetByteStreamIn()
	buf := make([]byte, 4)
...
func (r *LASreadItemCompressedByte14v3) Init(item []byte, ctx *uint32) error {
	instream := r.dec.GetByteStreamIn()
...
func (r *LASreadItemCompressedByte14v3) createAndInitModelsDecompressors(ctx uint32, item []byte) {
	c := &r.contexts[ctx]
	if !c.Unused {
		return
	}
...
// C++ original: LASreadItemCompressed_BYTE14_v3::read(U8* item, U32& context)
//
// v3 context-switch rule (matches C++ v3 bug): when switching to an
// already-used context, lastItem stays from the OLD context while models
// (c) come from the new context. Only on first visit (Unused) is lastItem
// updated to the new context's seed. This cross-context predictor behaviour
// is what the v3 encoder encoded against, so the decoder must match.
func (r *LASreadItemCompressedByte14v3) Read(item []byte, context *uint32) error {
	c := &r.contexts[r.currentContext]
	lastItem := c.LastItem
	...
	if r.currentContext != *context {
		...
		c = &r.contexts[r.currentContext] // always use new context's models
		// Note: if context was already used, lastItem still points to the OLD
		// context's backing array — matches C++ v3 behaviour.
	}
